...
The verified service runs under Xvfb without a visible Java window. Complete the externally offered IBKR second-factor flow when requested; the 1Password OTP field is not used. Use `ibkr-local connect --profile main-live` and the service journal as the health source of truth.
